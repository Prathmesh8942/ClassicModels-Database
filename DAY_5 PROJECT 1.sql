#  Project: SQL Queries on ClassicModels Database
# The ClassicModels database is a sample MySQL database.
use classicmodels;                  
#The classic models data base contain table for:
select * from customers;#salesRepEmployeeNumber  # customer analysis customer detail name adderss credit limit location of customer
select * from employees;#employeesnumber        # emplyeee analysis{ filtering employee ,country}
select * from offices;
select * from orderdetails;
select * from orders;                   # order analysis  join order and customers
select * from payments;                # payment analysis group by customers number + amount
select * from productlines; # productline
select * from products;                         # product analysis top 5 product

# key sedctions and explations:-
#####   Used to simulate a sales & order management system.Financial Performance,Sales Analysis
#Objective of the Project
#Practice SQL queries.
#Showcase joins, grouping, aggregations, and filtering.

#List the customers in the United States with a credit limit higher than \$1000.# customer analysis
...
--  Make another query to list the names and email addresses of those employees. # employee analysis
...
select c.customerNumber,customerName,e.employeeNumber as representativenumber,concat(e.firstName," ",e.lastName) as account_representative from customers c left join employees e on c.salesRepEmployeeNumber=e.employeenumber;
#Report total payments for Atelier graphique.
select C.customerNumber,C.customerName,sum(amount) as Total_payments from customers C left join 
payments P on C.customerNumber=P.customerNumber group by C.customerNumber,C.customerName having C.customerName="Atelier graphique";
...
select * from orderdetails;
select * from orders;
# This project demonstrates joins, aggregations, conditions, and date functions in SQL.
